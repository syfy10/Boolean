# Boollm capability platform

Boollm now exposes one zero-dependency platform layer for advanced agent work.
...
| Scheduled automation | Notepad selections and Settings create durable one-time, daily, weekly, or monthly reminders, answer-only AI follow-ups, page opens, commands, and HTTPS webhooks. Tasks persist across restarts, run while Boollm is open, catch up after launch, and retain bounded run history. |
| Documents | Boollm creates structurally verified DOCX, XLSX, PPTX, and PDF files without runtime dependencies. |
...
| Current research | Boollm searches in the background, ranks official, primary, government, academic, and first-party documentation sources before weaker sources, reads the strongest pages, and returns numbered evidence with direct citation URLs. |
...
- Google Workspace, Microsoft 365, and other services require their connector,
  OAuth setup, or a compatible MCP/skill; Boollm does not silently gain access.
