...
$small = [BoollmWin]::LoadImage([IntPtr]::Zero, $IconPath, 1, 16, 16, 0x0010)
$big   = [BoollmWin]::LoadImage([IntPtr]::Zero, $IconPath, 1, 32, 32, 0x0010)
if ($small -eq [IntPtr]::Zero -and $big -eq [IntPtr]::Zero) { return }

$apply = {
  param($h)
  if ($small -ne [IntPtr]::Zero) { [BoollmWin]::SendMessage($h, 0x80, [IntPtr]0, $small) | Out-Null } # WM_SETICON small
  if ($big   -ne [IntPtr]::Zero) { [BoollmWin]::SendMessage($h, 0x80, [IntPtr]1, $big)   | Out-Null } # WM_SETICON big
}

# retry: the app window may not exist yet right after launch
for ($attempt = 0; $attempt -lt 40; $attempt++) {
  $found = $false
  $cb = [BoollmWin+EnumProc]{
    param($h, $l)
    if (-not [BoollmWin]::IsWindowVisible($h)) { return $true }
    $t = New-Object System.Text.StringBuilder 256
    [BoollmWin]::GetWindowText($h, $t, 256) | Out-Null
    $title = $t.ToString()
    if ($title -eq $Title -or $title -like "$Title*") {
      $c = New-Object System.Text.StringBuilder 128
      [BoollmWin]::GetClassName($h, $c, 128) | Out-Null
      if ($c.ToString() -like "Chrome_WidgetWin*") { & $apply $h; $script:found = $true }
    }
    return $true
  }
  $script:found = $false
  [BoollmWin]::EnumWindows($cb, [IntPtr]::Zero) | Out-Null
